...
/**
 * "Hello and Welcome" เล่าด้วยการเลื่อน — ริบบิ้นเวกเตอร์ (SVG) ไม่ใช่ 3D
 *
 ...
 *
 * ของเรา: สามบีต HELLO · AND · WELCOME — แต่ละบีตเป็นริบบิ้น "คนละแผ่น" แผ่นใหม่เข้ามาจากล่าง
 ...
 *
 * เงื่อนไขของการเลือก: พื้นจอนี้เป็น "ขาว" ต่อจากเมฆที่ถมจอ hero (ดู components/CloudWipe) ทุกแถบ
 * จึงต้องเข้มพอให้เห็นเป็นแผ่นบนพื้นขาว — สีอ่อนอย่างทราย #e2d7cb ใช้เป็นพื้นแถบไม่ได้ มันจมพื้น
 * เหลือ ส้ม/ดำหมึก/เขียวเข้ม ตัวอักษรใช้ ink อ่อนทั้งสามแถบ
 *
 * แถบกลางเป็นดำหมึก (--v3-shell-ink) ไม่ใช่น้ำเงิน: ตอนจบสามท่อนต่อกันเป็นแถบคั่นบนพื้น
 * น้ำเงินของจอถัดไป ท่อนน้ำเงินจะจมหายไปกับพื้น เหลือช่องว่างกลางแถบ
 */
const BEATS = [
  { word: 'HELLO', bg: '#fd5000', ink: '#f0f0f0', speed: 0.9, phase: 0.0 },
  { word: 'AND', bg: '#16181f', ink: '#f0f0f0', speed: 1.15, phase: 2.1 },
  { word: 'WELCOME', bg: '#01754f', ink: '#f0f0f0', speed: 0.75, phase: 4.0 },
]
...
export function OpenToWorkRibbon({ id = 'open-to-work' }: { id?: string }) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      /**
       * พื้นเป็นขาว (ต่อจากเมฆที่ถมจอ hero) แต่ "ทึบขึ้นเรื่อย ๆ" ตอนเข้าเรื่อง ไม่ใช่แผ่นทึบที่เลื่อนมาทับ
       *
      ...
      if (stage.current) {
        ...
        stage.current.style.background = `rgba(255, 255, 255, ${o.toFixed(3)})`
      }
