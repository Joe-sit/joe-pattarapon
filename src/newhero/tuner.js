...
export const DEFAULTS = {
  ...
  /**
   * เคอร์เซอร์ในฉาก — ตัวนี้คือตัวนำสายตาของทั้งหน้า
   *
   * มันเริ่มชีวิตอยู่ในฉากจอแรก (ท่าเข้าฉาก ตำแหน่งที่จูนไว้ หน้าหน้าต่าง) แล้วถูกส่งต่อ
   * ให้ชั้นนำสายตาของหน้าตอนเลื่อนพ้นจอแรก (ดู cursorguide/heroCursor) ปิดอันนี้ =
   * ไม่มีเคอร์เซอร์ให้ส่งต่อ การเดินทางทั้งเส้นก็หายไปด้วย
   */
  cu: 1,
  ...
  rbHover: 1.2,
  /**
   * ท้องฟ้าของฉาก — ไล่สีคำนวณต่อพิกเซลในเชดเดอร์ (ดู Backdrop ใน NewHeroScene)
   *
   * สีอยู่ในโค้ด (SKY_* ในไฟล์นั้น) ส่วนที่จูนได้คือตำแหน่ง/ขนาด/ความแรงของแสงแดด
   * ไอแดดที่ขอบฟ้า และการหรี่ขอบเฟรม — สโตร์นี้รับเฉพาะตัวเลข จึงไม่มีคีย์สี
   */
  skySunX: 0.62,
  skySunY: 0.72,
  skySunR: 0.1,
  ...
  flatHiEdge: 1.15,
  /**
   * ความนุ่มของรอยต่อระหว่างชั้นแสง — 0.5 คือสุดสเกล (ค่าที่จูนแล้วบนจอจริง)
   *
   * 0 = ขอบชั้นคมสนิทแบบเวกเตอร์ ซึ่งกัดเป็นรอยหยักตามขอบพอลิกอนบนผิวโค้ง
   * 0.5 = รอยต่อมีช่วงไล่ ยังอ่านเป็นสามชั้นอยู่ แต่ขอบไม่แตกเป็นขั้น
   */
  flatSoft: 0.5,
  flatShadow: 0.62,
  flatLit: 1,
  ...
  enT0: 0.28,
  enT1: 0.56,
  /** รูปกราฟความเร่ง: 0 = ความเร่งคงที่, 1 = เอ็กซ์โพเนนเชียล (ยิ่งเร็วยิ่งเร่ง) */
  enAccelExp: 1,
  ...
  etFade: 0.15,
  etShrink: 0.05,
  /** ความเร็วที่ถือว่า "เต็มรอย" (หน่วย/วินาที) — ช้ากว่านี้รอยสั้นลงตามสัดส่วน */
  etSpeed: 3,
  /** สัดส่วนที่ชั้นเงายังค้างอยู่ตอนตัวละครจอด (1 = ค้างครบ) */
  etHold: 1,
  /** ความฟุ้ง 0..1 — 0 = ซิลูเอตต์เกือบคม, 1 = กลุ่มสีที่เหลือแค่เค้าตัว (ดู EchoTrail) */
  etSoft: 0.7,
  etDbg: 0,
  pfxAmt: 0,
  pfxSize: 2.6,
   ...
   */
  enGrav: 0.25,
  /** ความเร็วตอนถึงปลายเป็นกี่เท่าของตอนออกจากพอร์ทัล (1 = ความเร็วคงที่) */
  enAccel: 5.5,
  enLean: 1,
  ...
  bdWheelW: 0.058,
  bdRideY: 0.13,
  /** ล้อหมุน (rad/s) ตอนตัวละคร idle — 0 = หยุด */
  bdSpin: 2.2,
  /** ลายหยดบนล้อ — ขนาดหยด (หน่วยโลก), สัดส่วนน้ำเงิน/ขาว, เมล็ดสุ่ม */
  bdBlob: 0.025,
  bdBlobMix: 0.52,
  bdBlobSeed: 1,
  bdWhDbg: 0,
  ...
  enCrouch: 0.62,
  /** ย่อค้างไว้เท่าไรตลอดช่วงไถล (ไม่เกี่ยวกับแรง) — คนเล่นสเก็ตย่อรอไว้ก่อนอยู่แล้ว */
  enCrouchBase: 0.3,
  /** ความแข็งของสปริงที่ขาใช้ตามแรง (สูง = ตามไว) */
  enCrouchK: 90,
  /** ตัวหน่วงของสปริง (ต่ำ = เด้งหลายรอบ) */
  enCrouchDamp: 13,

  /* ── ช่วงไหลท้าย hero (scroll) — ดู scrolly.js / CruiseRibbon ────────────── */
  /** สัดส่วนการเลื่อนที่ยอมให้กล้องถอยก่อน แล้วริบบิ้นค่อยเริ่มงอก */
  cruStart: 0,
  /** ช่วงการเลื่อน (เท่าของหนึ่งวิวพอร์ต) ที่กล้องขยับจนสุด */
  cruPull: 1,
  /**
   * กล้องขยับระหว่างช่วงไหลเท่าไร — ค่าเริ่มต้นคือ "ไม่ขยับ"
   *
   * กล้องลดระดับเล็กน้อยตอนเลื่อนพ้นจอแรก = พารัลแลกซ์ของฉาก ของที่อยู่ใกล้ (ตัวละคร
   * ริบบิ้น) เลื่อนขึ้นเร็วกว่าของที่อยู่ไกล (แถบหน้าต่าง) เพราะระยะลึกต่างกันจริง
   *
   * ไม่ถอยกล้อง (cruZoom = 0): องค์ประกอบทั้งจอถูกจูนจากเส้น perspective ของแบบ
   * ถอยเมื่อไรมาตราส่วนทั้งจอเปลี่ยน ของเลื่อนหลุดตำแหน่งกลางทาง เคยลองแล้วพัง
   */
  cruZoom: 0,
  cruDrop: -2.2,
  /**
   * ไถลได้ไกลสุดกี่ส่วนของทั้งเส้น ตอนเลื่อนพ้นจอแรกพอดี
   *
   * ตั้งไว้น้อย ๆ ตั้งใจ: ปลายเส้นหลักวิ่งเข้าหากล้องแล้วดำดิ่งออกขอบล่าง ไถลไปไกลกว่านี้
   * ตัวละครจะผ่านหน้าเลนส์ (โตคับจอแล้วโดนตัด) ก่อนจะได้เลี้ยวขึ้นไปทางขวาด้วยซ้ำ
   * ระยะที่มีให้เลื่อนคือวิวพอร์ตเดียว เรื่องที่เล่าได้จึงเป็น "ขยับตัวไปตามถนน" ไม่ใช่ทั้งเส้น
   */
  cruSpan: 0.08,
  /** ปลายที่วาดล้ำหน้าตัวละครเท่าไร — ไม่ให้ไถลอยู่บนที่ว่าง */
  cruLead: 0.05,
  /** หมุนตัวตามเส้นระหว่างไหลแค่ไหน (0 = ไม่หมุนเลย) */
  cruTurn: 1,
}
...
/**
 * ค่าเริ่มต้น *ชุดก่อน* ของบล็อกรอยเงา — ใช้ล้างค่าที่ค้างในเบราว์เซอร์ตอนความหมายเปลี่ยน
 *
 * etSoft เคยเป็นจำนวนพิกเซลที่ขยายหน้ากาก (0–20) ตอนนี้เป็นความฟุ้ง 0–1 ค่าที่ค้างอยู่ที่ 3
 * จึงถูกอ่านเป็น "ฟุ้ง 300%" = ไปอ่านมิปชั้นที่ภาพเหลือพิกเซลเดียว เงาเลยจางหายไปทั้งชุด
 * และระยะห่าง/จำนวนใบชุดก่อนก็ถูกจัดใหม่ไปแล้ว
 *
 * กฎ: ถ้าค่าที่ค้างตรงกับค่าเริ่มต้นชุดก่อนเป๊ะ = เจ้าของเบราว์เซอร์ไม่เคยจูนคีย์นั้นเอง
 * เอาค่าใหม่ไปได้เลย ถ้าต่าง = เขาตั้งไว้เอง เก็บของเขาไว้ (ยกเว้น etSoft ที่หน่วยเปลี่ยน
 * ไปเลย ค่านอกช่วง 0–1 ไม่มีความหมายในหน่วยใหม่)
 */
const STALE = { etCount: 4, etStep: 0.42, etFade: 0.12, etShrink: 0.16, etSoft: 3 }
...
function load() {
  ...
  try {
    ...
    for (const k of Object.keys(DEFAULTS)) {
      if (typeof saved[k] !== 'number') continue
      if (k in STALE && saved[k] === STALE[k]) continue
      if (k === 'etSoft' && (saved[k] < 0 || saved[k] > 1)) continue
      out[k] = saved[k]
    }
    return out
  } catch {
