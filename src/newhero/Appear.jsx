...
export function Appear({ at = 0, dur = 0.5, from = null, rise = 0, tilt = 0, over = 1.4, children, ...props }) {
  ...
  useFrame(() => {
     ...
     *
     * ป้อน u ที่หน่วงหัวแล้ว (smoothstep) เข้าไปแทน u ดิบ — ทั้ง outBack และ ease-out
     * กำลังสามมีความชันสูงสุดที่จุดเริ่ม (4.1 และ 3) ของจึงกระโดดจากศูนย์ไปเห็นเป็นก้อน
     * ภายในสองเฟรมแรก อ่านเป็น "ป๊อป" ไม่ใช่ "โต" ยิ่งชิ้นใหญ่อย่างบานหน้าต่างยิ่งชัด
     */
    const w = u >= 1 ? 1 : u * u * (3 - 2 * u)
    const e = u >= 1 ? 1 : over > 0 ? outBack(w, over) * 0.6 + (1 - Math.pow(1 - w, 3)) * 0.4 : 1 - Math.pow(1 - w, 3)
    o.visible = true
