...
/**
 * จังหวะโผล่ — ทีละใบ ไม่ใช่ทั้งชุดพร้อมกัน และคาบต้องมาจากตัวไถลเอง
 *
 * รูปของจังหวะถอดจากของจริงที่ githubuniverse.com (ยิงภาพช่วงโหลดทุก 120ms แล้วนับพื้นที่
 * สีสด): พื้นที่สีเพิ่มเป็นขั้น ๆ แล้วลดลงนิ่ง — 0.6 วิ มีเงาใบเดียวหลังตัว · 0.85 วิ มีสามใบ
 * และทั้งสามยัง *ใหญ่กว่าที่สุดท้ายมันไปอยู่* กลไกคือใบติดตัวมาก่อน ไล่ออกทางหางทีละใบ
 * แต่ละใบพุ่งเลยสล็อตแล้วผ่อนกลับ ไม่ใช่ไล่ความจางของทั้งชุดพร้อมกัน
 *
 * *คาบ* เป็นสัดส่วนของ enDur (ความยาวการไถลเข้าฉาก) ไม่ใช่วินาทีคงที่
 *
 * วัดจริงตอนเป็นเลขคงที่: ไถลจบที่ 3.39 วิ แต่พวงเงาเพิ่งครบ ~4.18 วิ — ช้ากว่าจุดที่ตัวละคร
 * จอดไปเกือบ 0.8 วิ นานพอให้อ่านเป็นแอนิเมชันคนละชิ้นที่มาต่อท้าย ไม่ใช่รอยที่การไถลทิ้งไว้
 * และถ้าใครไปปรับความยาวการไถล (enDur) ความสัมพันธ์ก็เพี้ยนทันทีเพราะเลขไม่ได้ผูกกัน
 *
 * POP_SPAN 0.25 = พวงเงาครบภายในหนึ่งในสี่ของเวลาที่ใช้ไถล (enDur 1.45 → 0.36 วิ) ใบแรก
 * ขึ้นเกือบทันทีที่หยุด (POP_RISE_F 0.11 ของ enDur ≈ 0.16 วิ) เหมือนของที่หลุดจากตัวตอนเบรก
 */
const POP_SPAN = 0.25
const POP_RISE_F = 0.11
...
function useEchoDebug({ gl, rt, state }) {
  ...
  useFrame(() => {
    ...
    o.txt.textContent =
      ...
      `(ครบใน ${(t.enDur * POP_SPAN).toFixed(2)}s = ${POP_SPAN}×enDur)\n` +
...
export function EchoTrail() {
  ...
  useFrame((_, delta) => {
    ...
    /** คาบทั้งหมด/ต่อใบ คิดจากความยาวการไถลของเฟรมนี้ — จูน enDur แล้วจังหวะนี้ตามไปเอง */
    const popAll = Math.max(0.08, t.enDur * POP_SPAN)
    const popRise = Math.max(0.05, t.enDur * POP_RISE_F)
    const popStep = count > 1 ? Math.max(0, (popAll - popRise) / (count - 1)) : 0
    ...
    for (let i = 0; i < MAX_ECHO; i += 1) {
      ...
      const ga = Math.min(1, Math.max(0, (gate.current - i * popStep) / popRise))
      m.visible = ga > 0.001
